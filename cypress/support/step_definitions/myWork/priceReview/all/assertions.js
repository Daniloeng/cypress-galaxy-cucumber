...
Then(`Only Price ReviewAll with this search bar filter {string} should displayed in the list`, (assetId) => {
  cy.wait('@filterBysearchBar', { timeout: 30000 }).get('table').find('td div[datacy="assetId"]').should('be.visible')
      .should('contain', assetId);
});

Then(`Only Price ReviewAll with this {string} AssetId should displayed in the list`, (assetId) => {
  cy.wait('@filterByAssetId', { timeout: 30000 }).get('table').find('td div[datacy="assetId"]').should('be.visible')
      .should('contain', assetId);
});

Then(`Only Price ReviewAll with this {string} Previous Id should displayed in the list`, (previousId) => {
...
Then(`Only Price ReviewAll with this {string} Portfolio should displayed in the list`, (portfolio) => {
  cy.wait('@filterByPortfolio', { timeout: 30000 }).get('table').find('td div[datacy="portfolio"]').should('be.visible')
      .should('contain', portfolio);
});

Then(`Only Price ReviewAll with this {string} Proposed Price should displayed in the list`, (proposedPrice) => {
...
Then(`Only Price ReviewAll with this {string} Type should displayed in the list`, (type) => {
  cy.wait('@filterBytype', { timeout: 30000 }).get('.p-selectable-row').should('be.visible')
      .should('contain', type);
});

Then(`Only Price ReviewAll with this {string} Status should displayed in the list`, (status) => {
  cy.wait('@filterByStatus', { timeout: 30000 }).get('.p-selectable-row').should('be.visible')
      .should('contain', status);
});

Then(`Only Price ReviewAll with this {string} Requester should displayed in the list`, (requester) => {
  cy.wait('@filterByRequester', { timeout: 30000 }).get('.p-selectable-row').should('be.visible')
      .should('contain', requester);
});

Then(`Only Price ReviewAll with this {string} Decisor should displayed in the list`, (decisor) => {

...
When(`Fills the sidebar form for creating porpuses on tab Legal Cases`, () => {    
    cy.get('[type="file"]', {timeout:15000}).selectFile('cypress/fixtures/files/docCreationLegalCase.pdf', {force:true});
    ...
    cy.get('.gx-select__menu-list', { timeout: 15000 }).should('be.visible').get('.gx-select__option', { timeout: 15000 }).eq(14).click({ force: true });
    cy.get('[class^="QuickAdd_autocomplete"]', { timeout: 15000 }).type('2600/15.5T8LSB');
    cy.wait(500);
    cy.get('.gx-select__menu-list', { timeout: 15000 }).should('be.visible').get('.gx-select__option', { timeout: 15000 }).contains('2600/15.5T8LSB').click({ force: true });
    ...
    cy.get('#title', {timeout:15000}).type(`TestAutomationSuccessLegalCase`);
});
...
When(`Fills the sidebar form for creating porpuses on tab Sent`, () => {    
    cy.get('[type="file"]', { timeout: 15000 }).selectFile('cypress/fixtures/files/docCreation.pdf', { force:true });
    ...
    cy.get('.gx-select__menu-list', { timeout: 15000 }).should('be.visible').scrollIntoView().get('.gx-select__option', { timeout: 15000 }).eq(7).click({ force:true });
    cy.get('#documentDirectionId > .gx-select__control', {timeout: 15000}).should('have.text', 'Sent', { timeout: 15000 });
    cy.get('#title', { timeout: 15000 }).type(`TestAutomationSuccessSent`);
});

...
When(`Fills the sidebar form for creating porpuses on tab Cashflow`, () => {    
    cy.get('[type="file"]', { timeout: 15000 }).selectFile('cypress/fixtures/files/docCreationCashflow.pdf', {force:true});
    ...
    cy.get('.gx-select__menu-list', { timeout: 15000 }).should('be.visible').get('.gx-select__option', { timeout: 15000 }).eq(5).click({ force: true });
    cy.get('[class^="QuickAdd_autocomplete"]', { timeout: 15000 }).type(1);
    cy.wait(500);
    cy.get('.gx-select__menu-list', { timeout: 15000 }).should('be.visible').get('.gx-select__option', { timeout: 15000 }).contains('VAZ DEPINA').click({ force: true });
    ...
    cy.get('#title', { timeout: 15000 }).type(`TestAutomationSuccessCashflow`);
});
